...
void TextDialog::preProcess() {
	...
	while (i < text.size()) {
		...
		if (gmenu2x->font->getTextWidth(row) > gmenu2x->platform->w - 15) {
			vector<string> words;
			split(words, row, " ");

			uint32_t numWords = words.size();
			//find the maximum number of rows that can be printed on screen
			while (gmenu2x->font->getTextWidth(row) > gmenu2x->platform->w - 15 && numWords > 0) {
				numWords--;
				row = "";
...
int TextDialog::drawText(vector<string> *text, int32_t firstCol, int32_t firstRow, uint32_t rowsPerPage) {
	...
	for (uint32_t i = firstRow; i < firstRow + rowsPerPage && i < text->size(); i++) {
		...
		if (text->at(i) == "----") { // draw a line
			gmenu2x->s->box(5, y - 1 + fh / 2, gmenu2x->platform->w - 10, 1, 255, 255, 255, 130);
			gmenu2x->s->box(5, y + fh / 2, gmenu2x->platform->w - 10, 1, 0, 0, 0, 130);
		} else {
...
void TextDialog::exec() {
	...
	while (true) {
		...
		if (gmenu2x->input->isActive(UP) && firstRow > 0) firstRow--;
		else if (gmenu2x->input->isActive(DOWN) && firstRow + rowsPerPage < text.size()) firstRow++;
		else if (gmenu2x->input->isActive(RIGHT) && firstCol > -1 * (lineWidth - gmenu2x->listRect.w) - 10) firstCol -= 30;
		else if (gmenu2x->input->isActive(LEFT) && firstCol < 0) firstCol += 30;
		else if (gmenu2x->input->isActive(PAGEUP) || (gmenu2x->input->isActive(LEFT) && firstCol == 0)) {
			if (firstRow >= rowsPerPage - 1)
				firstRow -= rowsPerPage - 1;
			else
				firstRow = 0;
		}
		else if (gmenu2x->input->isActive(PAGEDOWN) || (gmenu2x->input->isActive(RIGHT) && firstCol == 0)) {
			if (firstRow + rowsPerPage * 2 - 1 < text.size())
				firstRow += rowsPerPage - 1;
			else
				firstRow = max(0, text.size() - rowsPerPage);
		}
		else if (gmenu2x->input->isActive(SETTINGS) || gmenu2x->input->isActive(CANCEL)) return;
	}
}
