...
bool WallpaperDialog::exec() {
	...
	while (true) {
		...
		if (gmenu2x->input->isActive(UP)) {
			...
			selected++;
		} else if (gmenu2x->input->isActive(PAGEUP) || gmenu2x->input->isActive(LEFT)) {
			selected -= numRows;
			if (selected < 0) selected = 0;
		} else if (gmenu2x->input->isActive(PAGEDOWN) || gmenu2x->input->isActive(RIGHT)) {
			selected += numRows;
			if (selected >= wallpapers.size()) selected = wallpapers.size() - 1;
		} else if (gmenu2x->input->isActive(MENU) || gmenu2x->input->isActive(CANCEL)) {
			return false;
		} else if ((gmenu2x->input->isActive(SETTINGS) || gmenu2x->input->isActive(CONFIRM)) && wallpapers.size() > 0) {
