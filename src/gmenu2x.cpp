...
void GMenu2X::editLink() {
	...
	if (platform->cpu_max > platform->cpu_min && platform->cpu_step > 0) {
		sd.addSetting(new MenuSettingInt(this,		_("CPU Clock"),		_("CPU clock frequency when launching this link"), &linkClock, platform->cpu_menu, platform->cpu_min, platform->cpu_max, platform->cpu_step));
	}
