...
void MenuSettingRGBA::draw(int y) {
	this->y = y;
	MenuSetting::draw(y);
	gmenu2x->s->box(153, y + (gmenu2x->font->height()/2) - 6, 12, 12, value());
	gmenu2x->s->rectangle(153, y + (gmenu2x->font->height()/2) - 6, 12, 12, 0, 0, 0, 255);
	gmenu2x->s->write(gmenu2x->font, /*"R: "+*/ strR, 169, y + gmenu2x->font->height() / 2, VAlignMiddle);
	gmenu2x->s->write(gmenu2x->font, /*"G: "+*/ strG, 205, y + gmenu2x->font->height() / 2, VAlignMiddle);
	gmenu2x->s->write(gmenu2x->font, /*"B: "+*/ strB, 241, y + gmenu2x->font->height() / 2, VAlignMiddle);
	gmenu2x->s->write(gmenu2x->font, /*"A: "+*/ strA, 277, y + gmenu2x->font->height() / 2, VAlignMiddle);
}
...
void MenuSettingRGBA::drawSelected(int y) {
	if (editing) {
		...
		switch (selPart) {
			...
			default: color = gmenu2x->skinConfColor["selectionBg"]; break;
		}
		gmenu2x->s->box(x, y, 36, gmenu2x->font->height() + 1, color);
		gmenu2x->s->rectangle(x, y, 36, gmenu2x->font->height() + 1, 0, 0, 0,255);
	}
